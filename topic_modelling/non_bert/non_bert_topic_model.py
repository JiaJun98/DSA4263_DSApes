...
class TopicModel(BaseModel):
    ...
    Users should initiate the preprocess_dataset method first before using train 
    and/or predict functions to generate necessary files
    """
    def __init__(self, train_dataset = None, test_dataset = None, pickled_model = None,
                 topic_label = None, pickled_vectorizer = None, feature_engineer = "bow",
                 custom_print = True):
        """
        ...
        train_dataset: Dataset class
            Store dataset for model to be trained on. May be None if pickled_model is not None to
            train on test_dataset.
        ...
        self.topic_label = pd.read_csv(
            topic_label).iloc[:,0].tolist() if topic_label is not None else []
        self.training_vectorizer = pickle.load(
            open(pickled_vectorizer, 'rb')) if pickled_vectorizer is not None else None
        self.feature_engineer = feature_engineer
        self.custom_print = custom_print

    def train(self, training_model, num_of_topics, num_top_words,
              num_top_documents, train_output_path):
        """
        ...
        vectorizer_name = f"training_{self.feature_engineer}_vectorizer_{num_of_topics}.pk"
        ...
        topic_key_words = self.display_topics(training.components_,
                                              vectorizer[0].get_feature_names_out(),
                                              num_top_words, train_output_path, training_model,
                                              vectorizer)

        labelled_train_topics = pd.DataFrame({"Text": self.train_dataset.text,
    ...
    def predict(self, test_output_path, root_word_option, remove_stop_words):
        ...
        root_word_option: int
            Specify if each word should be stemmed, lemmatized or remain as its original
            word before feature engineering
            valid inputs: 0 (original words), 1 (stemmed words), 2 (lemmatized words)
        remove_stop_words: bool
            True if stop words should be removed before feature engineering. False if all
            words are kept for feature engineering.
        ...
        test_data_fitted = self.training_vectorizer.transform(test_input.apply(" ".join))
    ...
    def preprocess_dataset(self, replace_stop_words_list = None,
                           include_words = ['taste', 'flavor', 'amazon', 'price',
                                            'minute', 'time', 'year'],
                            exclude_words = ["not", "no", "least", "less", "last", "serious", "too",
        ...
        if self.train_dataset is not None:
            ...
            if self.feature_engineer == "bow":
                self.train_dataset.create_bow(root_word_option, remove_stop_words,
                                              lower_case, word_form, ngrams, max_doc, min_doc)

            elif self.feature_engineer == "tfidf":
                self.train_dataset.create_tfidf(root_word_option, remove_stop_words, lower_case,
        ...
        if self.test_dataset is not None:
            self.test_dataset.modify_stop_words_list(replace_stop_words_list, include_words,
                                                     exclude_words)

            if self.feature_engineer == "bow":
                self.test_dataset.create_bow(root_word_option, remove_stop_words, lower_case,
                                             word_form, ngrams, max_doc, min_doc)

            elif self.feature_engineer == "tfidf":
                self.test_dataset.create_tfidf(root_word_option, remove_stop_words, lower_case,
                                               word_form, ngrams, max_doc, min_doc)

    def display_topics(self, trained_topics, feature_names, num_top_words, train_output_path,
                       training_model, vectorizer):
        """
        ...
        Return topic key words in the form of list of list. Each element in the outer list
        represents one topic and each element in the nested list represents a topic key word.
    ...
    def get_input_text(self, dataset, root_word_option, remove_stop_words):
        ...
        if root_word_option == 0:
            ...
            raise ValueError("invalid root word option")
    ...
    def churn_eval_metrics(self, labelled_test, num_top_documents, test_output_path):
        ...
        Consist of 3 columns, "Topic label", "Sample text", "Prediction" 
...
# def test(test_dataset, feature_engineer, pickled_model, pickled_vectorizer, test_output_path,
#          topic_label, replace_stop_words_list, include_words, exclude_words, root_word_option,
#          remove_stop_words, lower_case, word_form, ngrams, max_doc, min_doc):
#     """
#     Predict on the test dataset based on the pickled model and vectorizer.
#     """
#     testModel = TopicModel(test_dataset=test_dataset, pickled_model=pickled_model,
#                            topic_label = topic_label, pickled_vectorizer= pickled_vectorizer,
#                            feature_engineer=feature_engineer)
#     testModel.preprocess_dataset(replace_stop_words_list, include_words, exclude_words,
#                                  root_word_option, remove_stop_words, lower_case, word_form,
...
if __name__ == "__main__":
    ...
    ngrams = (config_file['model'][model_choice]['ngrams_start'],
    ...
    if isTrainer:
        if isTester:
            ...
            train_dataset, test_dataset = Dataset(data_df), None
